<?php
/**
 * Accessible Cookie Notice Bot detection.
 *
 * @category Bots
...
/**
 * Accessible_Cookie_Notice_Bot_Detect class.
 *
 ...
 *
 * Crawlers & Exclusions last updated 8/22/2021.
 *
 * @category  Bots
 * @package   Accessible Cookie Notice
 * @author    Joe Dolson
 ...
 * @version   1.0
 */
class Accessible_Cookie_Notice_Bot_Detect {
	...
	public function set_http_headers( $http_headers = null ) {
		...
		// only save HTTP headers - in PHP, only _SERVER vars that start with HTTP_.
	...
	public function set_user_agent( $user_agent = null ) {
		if ( false === empty( $user_agent ) ) {
			...
			foreach ( $this->get_ua_http_headers() as $alt_header ) {
				if ( false === empty( $this->http_headers[ $alt_header ] ) ) {
	...
	public function get_headers_list() {
		$data = array(
			...
			// vodafone header: http://www.seoprinciple.com/mobile-web-community-still-angry-at-vodafone/24/.
			...
			// sometimes bots use a genuine user agent, but fill this header in with an email.
			'HTTP_FROM',
		);
